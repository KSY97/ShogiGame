...
class GameUI(tk.Frame): # 클래스는 보통 부모클래스가 뭔지를 넣는다.
    ...
    def turn_of_human(self, event):
        ...
        if (30 <= event.x and event.x <= 830) and (30 <= event.y and event.y <=660): # 장기판의 범위 안에 있으면
    ...
    def on_draw(self): # 그림 갱신 
        ...
        if 0 <= self.select and self.select < 90:
            self.draw_cursor(int(self.select % 3) * 100 + 30, int(self.select / 3) * 70 + 30, 75)
    ...
    def draw_cursor(self,x,y,size):
        self.c.create_line(x + 1, y + 1,x+size - 1, y + 1,width=4.0,fill = "red")
        self.c.create_line(x + 1, y + size - 1 , x + size - 1, y + size - 1 ,width=4.0,fill = "red")
        self.c.create_line(x + 1, y + 1, x + 1, y + size - 1,width=4.0,fill = "red")
        self.c.create_line(x + size - 1, y + 1 , x + size - 1, y + size - 1 ,width=4.0,fill = "red")
